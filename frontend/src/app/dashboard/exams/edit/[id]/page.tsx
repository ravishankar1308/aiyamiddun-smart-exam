...
import { apiGetExam, apiUpdateExam, apiGetAllMetadata } from '@/lib/api';
    ...
    useEffect(() => {
        if (id) {
            Promise.all([
                apiGetExam(id),
                apiGetAllMetadata()
            ]).then(([examData, metaData]) => {
                setExam(examData);
                setTitle(examData.title);
                setDescription(examData.description || '');
                setGradeId(examData.grade_id.toString());
                setSubjectId(examData.subject_id.toString());
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <header className="mb-6">
                ...
                <p className="text-gray-500">Update the details for the exam titled \"{exam.title}\".</p>
            </header>
