...
import { ArrowLeft, ArrowRight, Save, Printer, BrainCircuit, RefreshCw, ArrowUp, ArrowDown, X, FileText, Calendar } from 'lucide-react';
...
interface Subject extends MetadataItem {
    grade: string;
}
...
export default function NewExamPage() {
    ...
    const role = 'admin'; // Replace with actual role from context/auth

    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                setLoading(true);
                const [grades, subjects, sections, qTypes, questions] = await Promise.all([
                ...
                const parsedQuestions = questions.map(q => {
                    if (typeof q.options === 'string') {
                        try {
                            // Make sure to handle empty strings or invalid JSON
                            return { ...q, options: q.options ? JSON.parse(q.options) : [] };
                        } catch (e) {
                            console.error(`Failed to parse options for question ${q.id}:`, q.options, e);
                            return { ...q, options: [] }; // Fallback to empty array on error
                        }
                    }
                    // Ensure options is an array even if it's null or undefined from the API
    ...
    const handleGenerateDraft = () => {
        ...
        // 1. Create a mutable pool of questions filtered by global criteria
        ...
        // 2. Iterate and pick questions, removing them from the pool
        typesToPick.forEach(type => {
            ...
            mutablePool = nextPool; // Update pool for the next type
        });

        // 3. Set the final draft
        setExamDraft(draft.sort(() => 0.5 - Math.random())); // Final shuffle
    };

    const handleSaveExam = async () => {
        if (examDraft.length === 0) {
            alert("Cannot save an empty exam. Please generate a draft first.");
            return;
        }
        setLoading(true);
        try {
    ...
    const handleReplaceQuestion = (questionId: number) => {
        ...
        const pool = availableQuestions.filter(q => 
            ...
            q.section === questionToReplace.section // Same section
        );
    ...
    if (loading && availableQuestions.length === 0) return <p>Loading exam builder...</p>;
             ...
             <div className="grid lg:grid-cols-3 gap-8 print:block">
                <div className="lg:col-span-1 space-y-6 print:hidden">
                   <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
                      ...
                      <div><label className="text-xs font-bold text-slate-500 uppercase block mb-1">Exam Title</label><input type="text" value={examConfig.title} onChange={e => setExamConfig({...examConfig, title: e.target.value})} className="w-full p-2 border rounded text-sm" placeholder="e.g. Mid-Term 2024"/></div>
                      <div className="bg-blue-50 p-3 rounded border border-blue-100"><p className="text-xs font-bold text-blue-700 uppercase mb-2 flex items-center gap-1"><Calendar size={12}/> Quiz Schedule (Optional)</p><div className="grid grid-cols-1 gap-2 mb-2"><div><label className="text-[10px] font-bold text-slate-500 uppercase">Start Time</label><input type="datetime-local" value={examConfig.scheduledStart} onChange={e => setExamConfig({...examConfig, scheduledStart: e.target.value})} className="w-full p-1.5 border rounded text-xs"/></div><div><label className="text-[10px] font-bold text-slate-500 uppercase">End Time</label><input type="datetime-local" value={examConfig.scheduledEnd} onChange={e => setExamConfig({...examConfig, scheduledEnd: e.target.value})} className="w-full p-1.5 border rounded text-xs"/></div></div><p className="text-[10px] text-slate-400 italic">Set dates to enable Quiz Mode for students.</p></div>
                      <div><label className="text-xs font-bold text-slate-500 uppercase block mb-1">Duration (Mins)</label><input type="number" value={examConfig.duration} onChange={e => setExamConfig({...examConfig, duration: parseInt(e.target.value) || 0})} className="w-full p-2 border rounded text-sm" placeholder="60"/></div>
                      <div className="grid grid-cols-2 gap-3">
                         <div><label className="text-xs font-bold text-slate-500 uppercase block mb-1">Class</label><select value={examConfig.classLevel} onChange={e => setExamConfig({...examConfig, classLevel: e.target.value, subject: '', sections: [], sectionScopes: {}})} className="w-full p-2 border rounded text-sm"><option value="">Select</option>{activeGrades.map(g => <option key={g.id} value={g.name}>{g.name}</option>)}</select></div>
                         <div><label className="text-xs font-bold text-slate-500 uppercase block mb-1">Subject</label><select value={examConfig.subject} onChange={e => setExamConfig({...examConfig, subject: e.target.value, sections: [], sectionScopes: {}})} className="w-full p-2 border rounded text-sm" disabled={!examConfig.classLevel}><option value="">Select</option>{builderAvailableSubjects.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}</select></div>
                      </div>
                      ...
                      <div><label className="text-xs font-bold text-slate-500 uppercase block mb-1">Difficulty</label><select value={examConfig.difficulty} onChange={e => setExamConfig({...examConfig, difficulty: e.target.value})} className="w-full p-2 border rounded text-sm"><option value="">Any</option><option value="simple">Simple</option><option value="medium">Medium</option><option value="hard">Hard</option></select></div>
                      <button onClick={handleGenerateDraft} className="w-full bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 shadow-md"><BrainCircuit size={18}/> Generate Draft</button>
                   </div>
                </div>
                <div className="lg:col-span-2 relative">
                   {examDraft.length > 0 && (<div className="mb-4 flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200 shadow-sm print:hidden sticky top-0 z-10"><div className="flex gap-2 items-center"><span className="text-sm font-bold text-slate-600 bg-slate-100 px-3 py-1 rounded-full">{examDraft.length} Questions</span></div><div className="flex gap-2">{(role === 'admin' || role === 'owner') && (<button onClick={handleSaveExam} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-sm"><Save size={16}/> Save Exam</button>)}<button onClick={() => window.print()} className="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-sm"><Printer size={16}/> Print</button></div></div>)}
